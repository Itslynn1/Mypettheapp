                ...
                call.enqueue(new Callback<CreateUserResponseDto>() {
                    ...
                    public void onResponse(Call<CreateUserResponseDto> call, Response<CreateUserResponseDto> response) {
                        if (response.isSuccessful()) {
                            ...
                            Toast.makeText(ThirdActivity.this, "Регистрация успешна. ID пользователя: " + createUserResponseDto.getId(), Toast.LENGTH_SHORT).show();
                            Log.d("ThirdActivity", "Нажата кнопка регистрации");

                            // Вызов метода для перехода на следующую активность
                            startNewActivity(null); // null передается как фиктивный View
                        } else {
                            // Ошибка при регистрации
                            Toast.makeText(ThirdActivity.this, "Ошибка при регистрации", Toast.LENGTH_SHORT).show();
                            Log.d("ThirdActivity", "Нажата кнопка регистрации");
                        }
                    }
